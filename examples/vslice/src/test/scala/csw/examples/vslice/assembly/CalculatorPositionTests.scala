...
*/
    ...
    it("should allow one update") {
      ...
      val msg = fakeTromboneControl.expectMsgClass(classOf[HCDTromboneUpdate])
      msg should equal(HCDTromboneUpdate(stagePositionKey -> 94.0 withUnits stagePositionUnits))
    }
    ...
    it("should create a proper set of HCDPositionUpdate messages") {
      ...
      val totalElExpected = elevationTestValues.map(_._1).map(f => CalculationActor.naLayerElevation(calculationConfig, calculationConfig.defaultInitialElevation, f)).map(el => el + rangeExpected)
      ...
      val msgsExpected = totalElExpected.zip(encExpected).map(p => HCDTromboneUpdate(stagePositionKey -> p._1 withUnits stagePositionUnits))
    ...
    it("should create a proper published events from fake TCS flowing through Event Service to produce HCD encoder motion updates") {
      ...
      val calculatorActor = system.actorOf(CalculationActor.props(calculationConfig, Some(fakeTromboneControl.ref), None))
  ...
  describe("check output of calculator actor to the TromboneHCD") {
    ...
    it("should create a proper set of HCDPositionUpdate messages for focus error changes through HCD") {
      ...
      val rangeExpected = fevalues.map(f => CalculationActor.focusToRangeDistance(calculationConfig, f))
      ...
      val baseAltitude = CalculationActor.naLayerElevation(calculationConfig, calculationConfig.defaultInitialElevation, 0.0)
      // Create a set of expected elevation values so we can combine to get total elevation
      val totalElExpected = rangeExpected.map(f => CalculationActor.focusToRangeDistance(calculationConfig, f)).map(f => baseAltitude + f)
      // This uses the total elevation to get expected values for encoder position
      val encExpected = totalElExpected.map(f => TromboneControl.rangeDistanceTransform(TestControlConfig, pos(f)))
      println("encEx: " + encExpected)
