...
class ConfigDistributorActor extends Actor with ActorLogging {
   ...
   */
  private def submit(submit: SubmitWithRunId) {
    ...
    if (configState.done()) {
      runIdMap.get(configState.runId()) match {
        case Some(runId) =>
          ...
          checkIfDone(configState: ConfigState, runId)
        case None =>
   ...
   */
  def checkIfDone(configState: ConfigState, runId: RunId) {
   ...
   */
  def changeConfigState(runId: RunId, newConfigState: ConfigState, logName: String) {
    configParts.get(runId) match {
      case Some(configPartInfo) =>
