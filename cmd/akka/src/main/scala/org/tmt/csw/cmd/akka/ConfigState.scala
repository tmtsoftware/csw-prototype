...
object ConfigState {
  ...
  case class Submitted(runId: RunId) extends ConfigState {
    def stop() = false
    def done() = false
    def withRunId(newRunId: RunId)  = Submitted(newRunId)
  }

  case class Canceled(runId: RunId) extends ConfigState {
    def stop() = true
    def done() = true
    def withRunId(newRunId: RunId)  = Canceled(newRunId)
  }

  case class Aborted(runId: RunId) extends ConfigState {
    def stop() = true
    def done() = true
    def withRunId(newRunId: RunId)  = Aborted(newRunId)
  }

  case class Paused(runId: RunId) extends ConfigState {
    def stop() = true
    def done() = false
    def withRunId(newRunId: RunId)  = Paused(newRunId)
  }

  case class Resumed(runId: RunId) extends ConfigState {
    def stop() = false
    def done() = false
    def withRunId(newRunId: RunId)  = Resumed(newRunId)
  }

  case class Completed(runId: RunId) extends ConfigState {
    def stop() = false
    def done() = true
    def withRunId(newRunId: RunId)  = Completed(newRunId)
  }

  case class Error(runId: RunId) extends ConfigState {
    def stop() = false
    def done() = true
    def withRunId(newRunId: RunId)  = Error(newRunId)
  }
}
