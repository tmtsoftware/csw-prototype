...
import csw.util.param.Events.{EventInfo, ObserveEvent, StatusEvent, SystemEvent}
...
object ItemSetDsl {
   ...
   */
  def size[I <: Parameter[_]](item: I): Int = item.size
   ...
   */
  def units[I <: Parameter[_]](item: I): Units = item.units
   ...
   */
  def add[I <: Parameter[_], T <: ParameterSetType[T]](sc: T, item: I): T = sc.add(item)
   ...
   */
  def madd[I <: Parameter[_], T <: ParameterSetType[T]](sc: T, items: I*): T = sc.madd(items: _*)
   ...
   */
  def exists[S, I <: Parameter[S], T <: ParameterSetType[T]](sc: T, key: Key[S, I]): Boolean = sc.exists(key)
   ...
   */
  def remove[S, I <: Parameter[S], T <: ParameterSetType[T]](sc: T, key: Key[S, I]): T = sc.remove(key)
   ...
   */
  def remove[I <: Parameter[_], T <: ParameterSetType[T]](sc: T, item: I): T = sc.remove(item)
   ...
   */
  def item[S, I <: Parameter[S], T <: ParameterSetType[T]](sc: T, key: Key[S, I]): I = sc.item(key)
   ...
   */
  def get[S, I <: Parameter[S], T <: ParameterSetType[T]](sc: T, key: Key[S, I]): Option[I] = sc.get(key)
   ...
   */
  def get[S, I <: Parameter[S], T <: ParameterSetType[T]](sc: T, key: Key[S, I], index: Int): Option[S] = sc.get(key).flatMap((i: Parameter[S]) => i.get(index))
   ...
   */
  def head[S](item: Parameter[S]): S = item.head
   ...
   */
  def value[S](item: Parameter[S], index: Int): S = {
    item.value(index)
  }
   ...
   */
  def get[S](item: Parameter[S], index: Int): Option[S] = item.get(index)
   ...
   */
  def values[S](item: Parameter[S]): Vector[S] = item.values
   ...
   */
  def vset[S, I <: Parameter[S]](key: Key[S, I], v: Vector[S], units: Units = NoUnits): I = key.set(v, units)
   ...
   * @return a new item of the type associated with the key
   */
  def set[S, I <: Parameter[S]](key: Key[S, I], v: S*): I = key.set(v: _*)
   ...
   */
  def sc(info: CommandInfo, configKey: Prefix, items: Parameter[_]*): Setup = Setup(info, configKey).madd(items: _*)
   ...
   */
  def oc(info: CommandInfo, configKey: Prefix, items: Parameter[_]*): Observe = Observe(info, configKey).madd(items: _*)
   ...
   */
  def cs(configKey: Prefix, items: Parameter[_]*): StateVariable.CurrentState = StateVariable.CurrentState(configKey).madd(items: _*)
   ...
   */
  def ds(configKey: Prefix, items: Parameter[_]*): StateVariable.DemandState =
    StateVariable.DemandState(configKey).madd(items: _*)
   ...
   */
  def oe(eventInfo: EventInfo, items: Parameter[_]*): ObserveEvent = ObserveEvent(eventInfo).madd(items: _*)
   ...
   */
  def stEv(eventInfo: EventInfo, items: Parameter[_]*): StatusEvent = StatusEvent(eventInfo).madd(items: _*)
   ...
   */
  def sysEv(eventInfo: EventInfo, items: Parameter[_]*): SystemEvent = SystemEvent(eventInfo).madd(items: _*)
}