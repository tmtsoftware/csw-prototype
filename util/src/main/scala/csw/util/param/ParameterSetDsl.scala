...
object ParameterSetDsl {
   ...
   */
  def get[S, P <: Parameter[S], T <: ParameterSetType[T]](sc: T, key: Key[S, P], index: Int): Option[S] = sc.get(key).flatMap((i: Parameter[S]) => i.get(index))
   ...
   */
  def cs(prefix: Prefix, parameters: Parameter[_]*): StateVariable.CurrentState = StateVariable.CurrentState(prefix).madd(parameters: _*)
