...
import csw.util.cfg_old.ConfigValues.{ CValue, ValueData }
import csw.util.cfg_old.UnitsOfMeasure.Units
      ...
      case JsObject(root) ⇒
        root(SETUP) match {
          case JsObject(configFields) ⇒
            ...
            val prefix = configFields.keys.filter(!reserved(_)).head
            SetupConfig(obsId, prefix, JsValueToValueSet(configFields(prefix)))
...
private object ConfigJsonFormats {
  ...
  import csw.util.cfg_old.UnitsOfMeasure.NoUnits
