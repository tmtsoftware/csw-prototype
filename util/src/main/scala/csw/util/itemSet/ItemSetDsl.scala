...
import csw.util.itemSet.Events.{EventInfo, ObserveEvent, StatusEvent, SystemEvent}
...
object ItemSetDsl {
   ...
   */
  def add[I <: Item[_], T <: ItemSet[T]](sc: T, item: I): T = sc.add(item)
   ...
   */
  def madd[I <: Item[_], T <: ItemSet[T]](sc: T, items: I*): T = sc.madd(items: _*)
   ...
   */
  def exists[S, I <: Item[S], T <: ItemSet[T]](sc: T, key: Key[S, I]): Boolean = sc.exists(key)
   ...
   * @return the number of items in the configuration
   */
  def csize[T <: ItemSet[_]](sc: T): Int = sc.size
   ...
   */
  def remove[S, I <: Item[S], T <: ItemSet[T]](sc: T, key: Key[S, I]): T = sc.remove(key)
   ...
   */
  def remove[I <: Item[_], T <: ItemSet[T]](sc: T, item: I): T = sc.remove(item)
   ...
   */
  def item[S, I <: Item[S], T <: ItemSet[T]](sc: T, key: Key[S, I]): I = sc.item(key)
   ...
   */
  def get[S, I <: Item[S], T <: ItemSet[T]](sc: T, key: Key[S, I]): Option[I] = sc.get(key)
   ...
   */
  def get[S, I <: Item[S], T <: ItemSet[T]](sc: T, key: Key[S, I], index: Int): Option[S] = sc.get(key).flatMap((i: Item[S]) => i.get(index))
   ...
   */
  def sc(info: ItemSetInfo, configKey: ItemSetKey, items: Item[_]*): Setup = Setup(info, configKey).madd(items: _*)
   ...
   */
  def oc(info: ItemSetInfo, configKey: ItemSetKey, items: Item[_]*): Observe = Observe(info, configKey).madd(items: _*)
   ...
   */
  def cs(configKey: ItemSetKey, items: Item[_]*): StateVariable.CurrentState = StateVariable.CurrentState(configKey).madd(items: _*)
   ...
   */
  def ds(configKey: ItemSetKey, items: Item[_]*): StateVariable.DemandState =
