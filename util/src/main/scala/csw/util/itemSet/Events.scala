...
object Events {
   ...
   */
  case class EventInfo(source: ItemSetKey, eventTime: EventTime, obsId: Option[ObsId], eventId: String = UUID.randomUUID().toString) {
  ...
  object EventInfo {
    implicit def apply(prefix: String): EventInfo = {
      val configKey: ItemSetKey = prefix
   ...
   */
  sealed trait EventType[T <: EventType[T]] extends ItemSet[T] with ItemSetKeyData {
    self: T =>
    ...
    override def itemSetKey: ItemSetKey = info.source

    /**
     * The event source is the prefix
     */
    def source: String = itemSetKey.prefix
   ...
   */
  case class StatusEvent(info: EventInfo, items: ItemsData = Set.empty[Item[_]])
      extends EventType[StatusEvent] with EventServiceEvent {

    // Java API
    def this(prefix: String) = this(EventInfo(prefix))

    override def create(data: ItemsData) = StatusEvent(info, data)
   ...
   * @param items an optional initial set of items (keys with values)
   */
  case class ObserveEvent(info: EventInfo, items: ItemsData = Set.empty[Item[_]])
      extends EventType[ObserveEvent] with EventServiceEvent {

    // Java API
    def this(prefix: String) = this(EventInfo(prefix))

    override def create(data: ItemsData) = ObserveEvent(info, data)
   ...
   * @param items an optional initial set of items (keys with values)
   */
  case class SystemEvent(info: EventInfo, items: ItemsData = Set.empty[Item[_]])
      extends EventType[SystemEvent] with EventServiceEvent {

    // Java API
    def this(prefix: String) = this(EventInfo(prefix))

    override def create(data: ItemsData) = SystemEvent(info, data)
