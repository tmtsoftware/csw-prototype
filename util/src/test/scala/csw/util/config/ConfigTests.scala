...
class ConfigTests extends FunSpec {
  ...
  describe("SC Basic Tests") {
    ...
    it("Should allow adding keys using single set") {
      val i1 = k1.set(22)
      val i2 = k2.set("A")
      val sc1 = SetupConfig(ck3).add(i1).add(i2)
      assert(sc1.size == 2)
  ...
  describe("OC Test") {
    ...
    it("Should allow adding keys") {
      val i1 = k1.set(22)
      val i2 = k2.set(44)
      val oc1 = ObserveConfig(ck3).add(i1).add(i2)
      assert(oc1.size == 2)
  ...
  describe("test setting multiple values") {
    val t1 = IntKey("test1")
    it("should allow setting a single head") {
      val i1 = t1.set(1)
  ...
  describe("test ObserveConfigArg") {
    ...
    val sc1 = ObserveConfig(ck1).madd(encoder1.set(22), encoder2.set(33))
    val sc2 = ObserveConfig(ck1).madd(xOffset.set(1), yOffset.set(2))
    assert(sc1.get(xOffset).isEmpty)
    assert(sc1.get(yOffset).isEmpty)
    assert(sc2.get(encoder1).isEmpty)
    assert(sc2.get(encoder2).isEmpty)
    assert(sc2.get(xOffset).isDefined)
    assert(sc2.get(yOffset).isDefined)
    assert(sc1.get(encoder1).isDefined)
    assert(sc1.get(encoder2).isDefined)
  ...
  describe("Check for multi-add") {
    ...
    it("Should allow vararg add") {
      val sc = SetupConfig(ck1).madd(i1, i2, i3)
      assert(sc.size == 3)
      assert(sc.exists(k1))
      assert(sc.exists(k2))
      assert(sc.exists(k3))
