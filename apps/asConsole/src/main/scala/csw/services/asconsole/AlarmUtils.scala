...
/**
 * Utility methods for working with the Alarm Service Redis database
 */
object AlarmUtils {
   ...
   */
  def initAlarms(redisClient: RedisClient, inputFile: File)(implicit ec: ExecutionContext): Future[List[Problem]] = {
    ...
    val problems = validate(inputConfig, jsonSchema, inputFile.getName)
    if (Problem.errorCount(problems) != 0) {
      ...
      val f = alarms.map { alarm ⇒
        redisClient.hmset(alarm.key(), alarm.map())
      }
   ...
   * @param redisClient used to access redis
   * @param key the key for the alarm in redis
   * @return the alarm model object
   */
  def getAlarm(redisClient: RedisClient, key: String)(implicit ec: ExecutionContext): Future[AlarmModel] = {
   ...
   */
  private def toJson(file: File): String = {
   ...
   * @return a list of problems, if any were found
   */
  private def validate(inputFile: File, schemaFile: File): List[Problem] = {
