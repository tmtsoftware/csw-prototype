...
case class Angle(uas: Long) extends AnyVal with Ordered[Angle] {
...
object Angle {
  ...
  protected[coords] val CIRCLE: Long = 360l * 60l * 60l * 1000l * 1000l

  /** used in implicit conversion to support `1.degree`, `1.arcMinute` etc */
  protected class AngleWrapperDouble(d: Double) {
  ...
  implicit protected[coords] def long2angle(d: Long) = new AngleWrapperDouble(d)

  implicit protected[coords] def int2angle(d: Int) = new AngleWrapperDouble(d)

  implicit protected[coords] def double2angle(d: Double) = new AngleWrapperDouble(d)
