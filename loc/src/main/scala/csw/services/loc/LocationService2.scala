...
object LocationService2 {
  ...
  def props(serviceRefs: Set[ServiceRef], replyTo: Option[ActorRef] = None): Props =
    Props(classOf[LocationService2], serviceRefs, replyTo)
  ...
  def resolve(serviceRefs: Set[ServiceRef])(implicit system: ActorSystem, timeout: Timeout): Future[ServicesReady] = {
    ...
    import system.dispatcher
    val actorRef = system.actorOf(LocationServiceWorker2.props())
    val f = (actorRef ? LocationServiceWorker2.Request(serviceRefs)).mapTo[ServicesReady]
    f.onComplete {
...
/**
  ...
  * @param replyTo optional actorRef to reply to (default: parent of this actor)
  */
case class LocationService2(serviceRefs: Set[ServiceRef], replyTo: Option[ActorRef] = None)
...
/**
  ...
  * gets a result.
  */
protected object LocationServiceWorker2 {
  case class Request(serviceRefs: Set[ServiceRef])

  def props(): Props = Props(classOf[LocationServiceWorker2])
}

protected class LocationServiceWorker2 extends Actor with ActorLogging {
  import LocationServiceWorker2._
