...
/**
  * TMT Source Code: 3/7/16.
  */
class ContainerComponentTests extends TestKit(ActorSystem()) with ImplicitSender
  ...
  it("Should have container fields") {
    ...
    assert(parseDuration(containerName, CREATION_DELAY, setup, 5.seconds) == 2.seconds)
    assert(parseDuration(containerName, INITIAL_DELAY, setup, 5.seconds) == 1.seconds)
    assert(parseDuration(containerName, LIFECYCLE_DELAY, setup, 5.seconds) == 4.seconds)
  ...
  it("Should create a new Ass from config") {
    ...
    val cc  = ContainerComponent.create(setup2)
