...
object SupervisorTests {
  ...
  case class SimpleTestHcd(override val info: HcdInfo, supervisorIn: ActorRef) extends Hcd with LifecycleHandler {
    def receive = lifecycleHandlerReceive
  }

  case class SimpleTestAssembly(override val info: AssemblyInfo, supervisorIn: ActorRef) extends Assembly with LifecycleHandler {
    def receive = lifecycleHandlerReceive
  }
