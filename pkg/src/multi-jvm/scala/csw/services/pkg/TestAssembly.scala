...
import csw.util.cfg.Configurations.{SetupConfig, ObserveConfigArg, ControlConfigArg, SetupConfigArg}
...
case class TestAssembly(name: String) extends Assembly with AssemblyController with LifecycleHandler {
  ...
  /**
    * Decide if a config can be accepted
    */
  override protected def validate(config: ControlConfigArg): Validation = {
    ...
    config match {
      case setupConfigArg: SetupConfigArg =>
        // XXX insert code here
        val list = setupConfigArg.configs.map(validateConfig).filter(!_.isValid)
        if (list.nonEmpty) list.head else Valid

      case _: ObserveConfigArg =>
        Invalid("Was not expecting an observe config")
    }
  }
