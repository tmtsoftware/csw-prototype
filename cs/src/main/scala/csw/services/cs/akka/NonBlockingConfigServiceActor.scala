...
/**
 * A config service actor that uses futures to avoid blocking the queue
 *
 ...
 */
object NonBlockingConfigServiceActor {
  ...
  def props(configManager: NonBlockingConfigManager): Props =
    Props(classOf[NonBlockingConfigServiceActor], configManager)
 ...
 * @param configManager the configManager to use (See [[NonBlockingConfigServiceActor.defaultConfigManager]])
 */
class NonBlockingConfigServiceActor(configManager: NonBlockingConfigManager) extends Actor with ActorLogging {
