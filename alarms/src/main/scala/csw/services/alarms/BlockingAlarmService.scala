...
case class BlockingAlarmService(alarmService: AlarmService)(implicit val timeout: Timeout, context: ActorRefFactory) {
   ...
   */
  def monitorAlarms(alarmKey: AlarmKey, subscriber: ActorRef, notifyAll: Boolean): AlarmMonitor = alarmService.monitorAlarms(alarmKey, subscriber, notifyAll)
  ...
  def monitorAlarms(alarmKey: AlarmKey, notifyAlarm: AlarmStatus => Unit, notifyHealth: HealthStatus => Unit, notifyAll: Boolean): AlarmMonitor =
    alarmService.monitorAlarms(alarmKey, notifyAlarm, notifyHealth, notifyAll)
