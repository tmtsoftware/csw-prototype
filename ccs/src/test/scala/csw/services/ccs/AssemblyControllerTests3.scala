...
import csw.services.ccs.AssemblyController3.Submit
import csw.services.ccs.CommandStatus2.CommandResponse
import csw.services.ccs.Validation.{Valid, Validation}
import csw.util.config.Configurations3.{ItemSetInfo, Setup}
...
object AssemblyControllerTests3 {
  ...
  object TestAssemblyController3 {
    def props(): Props = Props(classOf[TestAssemblyController3])
  }

  class TestAssemblyController3 extends AssemblyController3 with Actor with ActorLogging {
  ...
  class TestWorker extends Actor with ActorLogging {
    ...
    def receive: Receive = {
      ...
      case WorkDone(config, replyTo) =>
        ...
        replyTo.map(_ ! CommandStatus2.Completed)
...
//@DoNotDiscover
class AssemblyControllerTests3 extends TestKit(AssemblyControllerTests3.system)
  with ImplicitSender with FunSuiteLike with LazyLogging {

  import AssemblyControllerTests3._


  test("Test Assembly controller") {
    val assemblyController = system.actorOf(TestAssemblyController3.props())
