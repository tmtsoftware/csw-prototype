...
import csw.services.loc.{LocationService, Connection}
 ...
 *
 * The list of possible target assemblies must be passed in the connections argument.
 *
...
object DistributorController {
   ...
   *
   * @param connections set of services required
   * @param timeout Timeout while waiting for all configs to complete (default: 10 secs)
   * @return
   */
  def props(connections: Set[Connection], timeout: Timeout = Timeout(10.seconds)): Props =
    Props(classOf[DistributorController], connections, timeout)

}

protected case class DistributorController(connections: Set[Connection], timeout: Timeout) extends Actor with ActorLogging {
  ...
  def connected(services: Map[Connection, ResolvedService]): Receive = {
   ...
   */
  private def process(services: Map[Connection, ResolvedService], configArg: SetupConfigArg): Unit = {
