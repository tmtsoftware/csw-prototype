...
import csw.util.config.Configurations3.Result
...
object CommandStatus2 {
  ...
  object Invalid {
    ...
    def apply(in: Validation.Invalid): CommandStatus2.Invalid = new CommandStatus2.Invalid(in.issue)
  }

  /**
   * Java API: This is present to support returning a Validation as a CommandStatus
   */
  def createInvalid(in: Validation.Invalid): CommandStatus2.Invalid = Invalid(in)
  ...
  def validationAsCommandStatus(validation: Validation): CommandResponse = {
    validation match {
      case Validation.Valid        => CommandStatus2.Accepted
